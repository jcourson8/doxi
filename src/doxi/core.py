...
class DoxiScraper:
    def __init__(self, api_key, max_requests_per_minute=200, max_concurrent_requests=2, flat=False, force=False):
    ...
    async def process_link(self, session: aiohttp.ClientSession, link: str, file_path: str, sem: asyncio.Semaphore, rate_limit_interval: float, pbar):
    ...
    async def process_documentation_site(self, url: str, sem: asyncio.Semaphore, rate_limit_interval: float, output_dir: str = '.'):
        ...
        for link in nav_links:
            ...
            if self.flat:
                ...
                sub_dirs = os.path.join(*path_segments[:-1]) if len(path_segments) > 1 else ''
                save_path = os.path.join(base_folder_name, sub_dirs)
                create_folder(save_path)
                # Use the last segment as the filename
                file_name = sanitize_filename(path_segments[-1] if path_segments[-1] else 'index') + ".md"
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            for link, file_path in links_to_process:
                ...
                task = asyncio.create_task(self.process_link(session, link, file_path, sem, rate_limit_interval, pbar))
                tasks.append(task)
            ...
            try:
                ...
                print(f"\nError: {e}")
                print("The rate limit has been exceeded. Please consider obtaining an API key from https://jina.ai/reader/ for higher rate limits.")
    ...
    async def run(self, documentation_urls, output_dir='.'):
        ...
        for url in documentation_urls:
            task = asyncio.create_task(self.process_documentation_site(url, sem, rate_limit_interval, output_dir))
            tasks.append(task)

        try:
            await asyncio.gather(*tasks)
        except RateLimitExceededError:
            print("Stopping the scraping process due to rate limit being exceeded.")
        except Exception as e:
